...
def parse_args():
    ...
    parser.add_argument("--use_parden", action="store_true", help="for evaluation")
                ...
                with open(os.path.join(eval_dir, scenario + ".jsonl"), "r") as eval_file:
                    for line in eval_file:
                        if line.strip():  # Check if the line is not empty                        
                            ...
                            if args.use_parden:
                                instructions = eval_entry['response']['body']['choices'][0]['message']['content']
                                is_safe = eval_entry['eval_res']

                                if eval_entry['eval_res']['metrics']['llama_guard_unsafe']:
                                    process_per_prompt_for_repeat(
                                        eval_entry['custom_id'], # custom_id
                                        instructions, # only one target
                                        None, # image_base64,
                                        output_file,
                                        args.model,
                                        args.openai_key,
                                        ...
                                        args.max_tokens,
                                        eval_mode=True,
                                        image_pil=Image.new("RGB", (100, 100), (255,255,255)),
                                        is_safe=is_safe
                                    )
        ...
        if args.async_batch:
            ...
            for scenario in scenarios:
                ...
                for harmful_image in harmful_images:
                    ...
                    elif args.aug.startswith("twoshuffle"):
                        # non-labeling mixing augs
                        for harmless_image in harmless_images:
                            harmless_image_name = harmless_image[0]
                            harmless_image_path = harmless_image[1]
                            for harmful_alpha in args.lams:
                                base_custom_request_id = f"attack-{args.aug}-[Scenario]{scenario}-[HarmfulImg]{harmful_image_name}-[HarmlessImg]{harmless_image_name}-[HarmfulAlpha]{harmful_alpha}"
                                patch_num = int(args.aug.split("twoshuffle")[-1])
                                _augmented_images_pil = []
                                # for reproducibility of augmentations per run
                                for prompt_idx in range(len(scenario2prompts[scenario])):
                                    _augmented_image_pil = shuffle_two_image_patches(harmful_image_path, harmless_image_path, 
                                        alpha=harmful_alpha, patch_rows=patch_num, patch_cols=patch_num, 
                                        seed=string_to_hash(base_custom_request_id + f"-[PromptIdx]{prompt_idx}"))
                                    _augmented_images_pil.append(_augmented_image_pil)
                                    if prompt_idx == 0: _augmented_image_pil.save(f'outputs_{patch_num}_{patch_num}/two_{scenario}_alpha_{harmful_alpha}.png')
                                augmented_image_pil = _augmented_images_pil
